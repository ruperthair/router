...
/// finish()
/// ```
pub trait JSONSelectionVisitor<'a>: Sized {
    type Error;

    /// Visit an output key
    fn visit(&mut self, name: &'a str) -> Result<(), Self::Error>;

    /// Enter a subselection group
    /// Note: You can assume that the named selection corresponding to this
    /// group will be visited first.
    fn enter_group(&mut self, group: &'a str) -> Result<(), Self::Error>;
    ...
    /// returned.
    fn walk(mut self, selection: &'a JSONSelection) -> Result<(), Self::Error> {
...
mod tests {
    ...
    impl JSONSelectionVisitor<'_> for TestVisitor<'_> {
